...
namespace JenovaSpace 
{
	...
	public:
		JenovaSpace::rcode AddToPack(std::string sFile);

	public:
		JenovaSpace::rcode SavePack(std::string sFile);
		JenovaSpace::rcode LoadPack(std::string sFile);
		JenovaSpace::rcode ClearPack();

	public:
		JenovaSpace::ResourcePack::sEntry GetStreamBuffer(std::string sFile);
	...
	public:
		...
		Sprite(std::string sImageFile, JenovaSpace::ResourcePack *pack);
		Sprite(int32_t w, int32_t h);
		~Sprite();

	public:
		JenovaSpace::rcode LoadFromFile(std::string sImageFile, JenovaSpace::ResourcePack *pack = nullptr);
		JenovaSpace::rcode LoadFromPGESprFile(std::string sImageFile, JenovaSpace::ResourcePack *pack = nullptr);
		JenovaSpace::rcode SaveToPGESprFile(std::string sImageFile);
	...
	public:
		void SetSampleMode(JenovaSpace::Sprite::Mode mode = JenovaSpace::Sprite::Mode::NORMAL);
	...
	class JenovaExtension
	{
		friend class JenovaSpace::JenovaPixel;
	protected:
		static JenovaPixel* pge;
	};
...
namespace JenovaSpace
{
	...
	Sprite::Sprite(std::string sImageFile, JenovaSpace::ResourcePack *pack)
	{
	...
	JenovaSpace::rcode Sprite::LoadFromPGESprFile(std::string sImageFile, JenovaSpace::ResourcePack *pack)
	{
		if (pColData) delete[] pColData;
	...
	JenovaSpace::rcode Sprite::SaveToPGESprFile(std::string sImageFile)
	{
		if (pColData == nullptr) return JenovaSpace::FAIL;
	...
	JenovaSpace::rcode Sprite::LoadFromFile(std::string sImageFile, JenovaSpace::ResourcePack *pack)
	{
#ifdef _WIN32
		//Use GDI+
		...
		if (!f) return JenovaSpace::NO_FILE;
	...
	void Sprite::SetSampleMode(JenovaSpace::Sprite::Mode mode)
	{
		modeSample = mode;
	}
	...
	{
		if (modeSample == JenovaSpace::Sprite::Mode::NORMAL)
		{
	...
	Pixel Sprite::SampleBL(double u, double v)
	{
		...
		JenovaSpace::Pixel p3 = GetPixel(std::max(x, 0), std::min(y + 1, (int)height - 1));
		JenovaSpace::Pixel p4 = GetPixel(std::min(x + 1, (int)width - 1), std::min(y + 1, (int)height - 1));

		return JenovaSpace::Pixel(
	...
	JenovaSpace::rcode ResourcePack::AddToPack(std::string sFile)
	{
	...
	JenovaSpace::rcode ResourcePack::SavePack(std::string sFile)
	{
		std::ofstream ofs(sFile, std::ofstream::binary);
		if (!ofs.is_open()) return JenovaSpace::FAIL;
	...
	JenovaSpace::rcode ResourcePack::LoadPack(std::string sFile)
	{
		std::ifstream ifs(sFile, std::ifstream::binary);
		if (!ifs.is_open()) return JenovaSpace::FAIL;
	...
	JenovaSpace::ResourcePack::sEntry ResourcePack::GetStreamBuffer(std::string sFile)
	{
		return mapFiles[sFile];
	}

	JenovaSpace::rcode ResourcePack::ClearPack()
	{
		for (auto &e : mapFiles)
		{
	...
	JenovaPixel::JenovaPixel()
	{
		sAppName = "Undefined";
		JenovaSpace::JenovaExtension::pge = this;
	}

	JenovaSpace::rcode JenovaPixel::Construct(uint32_t screen_w, uint32_t screen_h, uint32_t pixel_w, uint32_t pixel_h, bool full_screen)
	{
...
//#endif
		//Start the thread
		bAtomActive = true;
		std::thread t = std::thread(&JenovaPixel::EngineThread, this);
	...
	void JenovaPixel::SetDrawTarget(Sprite *target)
	{
		if (target)
			pDrawTarget = target;
		else
	...
	Sprite* JenovaPixel::GetDrawTarget()
	{
		return pDrawTarget;
	}

	int32_t JenovaPixel::GetDrawTargetWidth()
	{
		if (pDrawTarget)
	...
	int32_t JenovaPixel::GetDrawTargetHeight()
	{
		if (pDrawTarget)
	...
	bool JenovaPixel::IsFocused()
	{
		return bHasInputFocus;
	}

	HWButton JenovaPixel::GetKey(Key k)
	{
		return pKeyboardState[k];
	}

	HWButton JenovaPixel::GetMouse(uint32_t b)
	{
		return pMouseState[b];
	}

	int32_t JenovaPixel::GetMouseX()
	{
		return nMousePosX;
	}

	int32_t JenovaPixel::GetMouseY()
	{
		return nMousePosY;
	}

	int32_t JenovaPixel::GetMouseWheel()
	{
		return nMouseWheelDelta;
	}

	int32_t JenovaPixel::ScreenWidth()
	{
		return nScreenWidth;
	}

	int32_t JenovaPixel::ScreenHeight()
	{
		return nScreenHeight;
	}

	bool JenovaPixel::Draw(int32_t x, int32_t y, Pixel p)
	{
	...
	void JenovaPixel::DrawLine(int32_t x1, int32_t y1, int32_t x2, int32_t y2, Pixel p, uint32_t pattern)
	{
	...
	void JenovaPixel::DrawCircle(int32_t x, int32_t y, int32_t radius, Pixel p, uint8_t mask)
	{
		int x0 = 0;
		int y0 = radius;
		int d = 3 - 2 * radius;
	...
	void JenovaPixel::FillCircle(int32_t x, int32_t y, int32_t radius, Pixel p)
	{
	...
	void JenovaPixel::DrawRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p)
	{
	...
	void JenovaPixel::Clear(Pixel p)
	{
		...
		for (int i = 0; i < pixels; i++)
			m[i] = p;
#ifdef OLC_DBG_OVERDRAW
		JenovaSpace::Sprite::nOverdrawCount += pixels;
#endif
	}

	void JenovaPixel::FillRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p)
	{
		int32_t x2 = x + w;
		int32_t y2 = y + h;
	...
	void JenovaPixel::DrawTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p)
	{
	...
	void JenovaPixel::FillTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p)
	{
	...
	void JenovaPixel::DrawSprite(int32_t x, int32_t y, Sprite *sprite, uint32_t scale)
	{
		if (sprite == nullptr)
			return;
	...
	void JenovaPixel::DrawPartialSprite(int32_t x, int32_t y, Sprite *sprite, int32_t ox, int32_t oy, int32_t w, int32_t h, uint32_t scale)
	{
		if (sprite == nullptr)
			return;
	...
	void JenovaPixel::DrawString(int32_t x, int32_t y, std::string sText, Pixel col, uint32_t scale)
	{
		int32_t sx = 0;
		int32_t sy = 0;
	...
	void JenovaPixel::SetPixelMode(Pixel::Mode m)
	{
		nPixelMode = m;
	}

	Pixel::Mode JenovaPixel::GetPixelMode()
	{
		return nPixelMode;
	}

	void JenovaPixel::SetPixelMode(std::function<JenovaSpace::Pixel(const int x, const int y, const JenovaSpace::Pixel&, const JenovaSpace::Pixel&)> pixelMode)
	{
	...
	bool JenovaPixel::OnUserDestroy()
	{
		return true;
	}
	//////////////////////////////////////////////////////////////////
...
#ifdef OLC_DBG_OVERDRAW
				JenovaSpace::Sprite::nOverdrawCount = 0;
#endif
	...
	void JenovaPixel::Jenova_ConstructFontSheet()
	{
		...
		fontSprite = new JenovaSpace::Sprite(128, 48);
		int px = 0, py = 0;
		for (int b = 0; b < 1024; b += 4)
		{
			...
			{
				int k = r & (1 << i) ? 255 : 0;
				fontSprite->SetPixel(px, py, JenovaSpace::Pixel(k, k, k, k));
				if (++py == 48) { px++; py = 0; }
			}
		}
	}
	...
	LRESULT CALLBACK JenovaPixel::Jenova_WindowEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		switch (uMsg)
		{
		case WM_CREATE:		sge = (JenovaPixel*)((LPCREATESTRUCT)lParam)->lpCreateParams;	return 0;
		case WM_MOUSEMOVE:
		{
	...
	JenovaSpace::JenovaPixel* JenovaSpace::JenovaExtension::pge = nullptr;
#ifdef OLC_DBG_OVERDRAW
